...
GpuPerformanceModel::RunTimes GpuPerformanceModel::EstimateRunTimes(
  ...
  for (const HloInstruction* u : fused_users) {
    ...
    // inputs.
    //
    // Consumer part can potentially be accounted for completeness.
    int64_t num_threads =
    ...
    absl::Duration input_access_type_by_this_consumer = ProducerInputAccessTime(
        cost_analysis, *device_info, producer, /*fused_consumer=*/u);
