...
// We think one sharding is straightly better than the other IFF any of the
// following holds true:
// 1. manual shardings are better than tile maximal shardings.
bool IsShardingStraightlyBetter(const HloSharding& lhs,
  ...
  if (lhs.IsTuple()) {
    ...
    for (int64_t i = 0; i < lhs_shardings.size(); ++i) {
      if (IsShardingStraightlyBetter(rhs_shardings[i], lhs_shardings[i])) {
        return false;
      }
      if (IsShardingStraightlyBetter(lhs_shardings[i], rhs_shardings[i])) {
        is_better = true;
      }
    }
    return is_better;
...
bool MaybeImproveInstructionSharding(HloSharding sharding,
  ...
  if (instruction->has_sharding() &&
      IsShardingStraightlyBetter(sharding, instruction->sharding())) {
