...
// Helper function to add a dimension at the beginning of a tensor of size
// extra_dim.
Shape ExpandShapeWithSize(int64_t extra_dim, const Shape& base_shape) {
  std::vector<int64_t> new_dimensions;
  new_dimensions.reserve(base_shape.dimensions_size() + 1);
  new_dimensions.push_back(extra_dim);
  new_dimensions.insert(new_dimensions.end(), base_shape.dimensions().begin(),
                        base_shape.dimensions().end());
  auto expanded_shape =
      ShapeUtil::MakeShape(base_shape.element_type(), new_dimensions);
...
Shape ComputeFullOutputShape(const WhileMoveInfo& move_info,
  ...
  std::vector<int64_t> new_dimensions;
  return ExpandShapeWithSize(move_info.dynamic_update_slice->operand(0)
                                 ->shape()
...
// concatenated afther the loop. Rough transformation: while (i < LAYERS) {
//   p0 = param(0)
//   p1 = param(1)
//   x = computation(p0)
//   xg = all-reduce(x)
...
Status TransformLoopForwardSink(const WhileLoopAnalysis& loop_analysis,
  ...
  // HloComputation* cond_computation = while_loop->while_condition();
  std::vector<HloInstruction*> new_init_operands;
  std::vector<Shape> new_parameter_shapes;
  std::vector<HloInstruction*> new_root_operands;
  ...
  for (auto& to_move : loop_analysis.GetMoveInfos()) {
    ...
    HloInstruction* to_insert =
        body_computation->AddInstruction(HloInstruction::CreateReshape(
            ExpandShapeWithSize(1,
                                new_root_operands[to_move.output_idx]->shape()),
