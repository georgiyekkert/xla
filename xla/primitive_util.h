...
ABSL_ATTRIBUTE_ALWAYS_INLINE inline constexpr int BitWidth(PrimitiveType type) {
  return PrimitiveTypeSwitch<int>(
      [type](auto primitive_type) -> int {
        if constexpr (IsArrayType(primitive_type)) {
          ...
          if constexpr (IsIntegralType(primitive_type)) {
            static_assert(std::numeric_limits<NativeT>::is_integer);
        ...
        if constexpr (primitive_type == TOKEN) {
          // Tokens require no space.
          return 0;
        }
        if constexpr (primitive_type == TUPLE) {
          LOG(FATAL) << "TUPLE is an invalid type for BitWidth";
        }
        if constexpr (primitive_type == OPAQUE_TYPE) {
...
ABSL_ATTRIBUTE_ALWAYS_INLINE inline constexpr int ByteWidth(
    PrimitiveType type) {
  return CeilOfRatio<size_t>(BitWidth(type),
