...
constexpr auto PrimitiveByteSize() {
  std::array<uint8_t, PrimitiveType_ARRAYSIZE> byte_size{};
  for (size_t i = 0; i < byte_size.size(); ++i) {
    const auto primitive_type = static_cast<PrimitiveType>(i);
    if (primitive_util::IsArrayType(primitive_type)) {
      byte_size[i] = primitive_util::ByteWidth(primitive_type);
    }
  }
  return byte_size;
}
constexpr auto primitive_byte_size = PrimitiveByteSize();
...
/* static */ bool ShapeUtil::FillNewShape(PrimitiveType element_type,
  ...
  int64_t dense_shape_size = ((eint >= 0 && eint < primitive_byte_size.size())
